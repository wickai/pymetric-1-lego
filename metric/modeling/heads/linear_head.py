...
class LinearHead(nn.Module):
    def __init__(self):
        ...
        if self.pool_type == "avgpool":
            ...
            self.pool_layer = nn.IdentiGty()
        else:
            raise KeyError(f"{self.pool_type} is invalid")
        ...
        if cls_type == "linear":
            self.classifier = nn.Linear(self.in_feat, self.num_classes, bias=False)
        elif cls_type == "arcface":
            self.classifier = Arcface(self.embedding_size, self.num_classes)
        elif cls_type == "circle":
            self.classifier = Circle(self.embedding_size, self.num_classes)
        else:
            raise KeyError(
                f"{cls_type} is invalid, please choose from "
